...
from flask import Flask, render_template, Response, request, redirect, url_for, json, session
...
path = "../NhanDIenKhuonMat_version2/static/imagesDB"
...
def SaveDB(msv, time):
    ...
    global ds_check_sv
    try:
        file_write = open(f"diemdanh{day}.txt", "a", encoding="utf-8")
            ...
            if model_check.User_name(name) and model_check.User_msv(msv_new):
                ...
                try:
                    if msv_old != msv_new:
                        ...
                        path = 'static/imagesDB/' + msv_old
                        for file in os.listdir(path):
                            file = file.split('_')
                            os.rename(path + '/' + file[0] + '_' + file[1], path + '/' + msv_new + '_' + file[1])
                        os.rename(path, f'static/imagesDB/{msv_new}')
                        # cập nhập ảnh
    ...
    if request.method == 'POST':
        if request.form.get('action') == 'Lưu và Xem ảnh':
            try:
                ...
                path_pic = f"static/imagesDB/{msv}"
