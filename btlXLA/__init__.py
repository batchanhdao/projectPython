        ...
        try:
            ...
            if method_authentication == 1:
                ...
                if len(faces) == 0:
                    ...
                    cv2.putText(frame, "close left eye", (poin_x, poin_y), cv2.FONT_HERSHEY_COMPLEX, 1,
                                (0, 0, 255), 2)

                    if check_blinking(landmarks, frame, "left"):
                ...
                if len(faces) == 0:
                    ...
                    cv2.putText(frame, "close right eye", (poin_x, poin_y), cv2.FONT_HERSHEY_COMPLEX, 1,
                                (0, 0, 255), 2)

                    if check_blinking(landmarks, frame, "right"):
                ...
                if len(faces) == 0:
                    ...
                    cv2.putText(frame, f"Open your mouth for about {time_open_mouth} seconds", (poin_x, poin_y),
...
#
# # Hàm để kiểm tra người dùng đã nhắm mắt hay chưa
# def check_blinking(landmarks, image):
#     # Lấy tọa độ của mắt trái và mắt phải
...
#     print("eyes", eyes)
#     # Trả về True nếu người dùng đã nhắm mắt
#     return round(eyes, 2) < 0.2
#
#
# # hàm kiểm tra người dùng nhắm mắt trái chưa
# def close_one_left_eye():
#
#
# # hàm kiểm tra người dùng nhắm mắt phải chưa
#
#
# # hàm kiểm tra người dùng mở miệng trong 3s chưa
#
...
#     print(mouth_aspect_ratio)
#     # Trả về True nếu người dùng đã mở miệng
#     return mouth_aspect_ratio > 10.0
#
#
# def identification(imgList, img_input_encode):
#     msv = ""
...
#                     past_face.append(image_input_encode)
#
#                 cv2.putText(frame, f"blinking eyes: {blinking_eyes}/{t}", (poin_x, poin_y), cv2.FONT_HERSHEY_COMPLEX,
#                             1, (0, 0, 255), 2)
#                 poin_y = poin_y + 40
#                 cv2.putText(frame, f"open mouth: {open_mouth}/{t}", (poin_x, poin_y), cv2.FONT_HERSHEY_COMPLEX, 1,
#                             (0, 0, 255), 2)
...
#
#                 # Xác thực thành công thì nhận dạng đó là ai
#                 if blinking_eyes == t and open_mouth == t:
#                     msv = identification(imgList, image_input_encode)
#                     cv2.putText(frame, f"authenticated: {msv}", (poin_x, poin_y), cv2.FONT_HERSHEY_COMPLEX, 1,
#                                 (0, 0, 255), 2)
#
#                 # Xác thực
#                 else:
#                     present_eyes = check_blinking(landmarks, frame)
#                     present_mouth = check_mouth(landmarks, frame)
#                     # open/close eyes
#                     if blinking_eyes != t and present_eyes != past_eyes:
#                         past_eyes = present_eyes
